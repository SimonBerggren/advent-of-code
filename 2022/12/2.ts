...
let possibleStartPoints: GridNode[] = [];
let end: GridNode;

const grid = input.split('\n').map((row, y) =>
    row.split('').map((char, x) => {
        if (char === 'S' || char === 'a') {
            possibleStartPoints.push({ coord: `${x}.${y}` });
...
const answers = possibleStartPoints.map((start) => run(start, end));
const shortestPath = answers.sort((a, b) => a.length - b.length).filter((arr) => arr.length);

console.log('Answer', shortestPath[0].length - 1);
