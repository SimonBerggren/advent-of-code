...
const input: string = readFileSync(resolve(__dirname, 'input.txt'), 'utf8');
...
const pathCosts = Object.fromEntries(
    ['AA', ...allValves].map((start) => [
        start,
        Object.fromEntries(
            allValves.filter((end) => end !== start).map((end) => [end, getLowestCost(start, end)])
        ),
    ])
);
    ...
    const bestCombo = candidates.reduce((best, [path, score], i) => {
        ...
        const splitPoint =
            best === 0
                ...
                : Math.max(
                      0,
                      candidates.findIndex((candidate) => candidate[1] + score < best) + 1 - i
                  );
...
wrapInTime('Part One', () => console.log('Part One:', scorePaths(makeAllPaths(30), 30)[0][1]));
